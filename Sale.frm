...
Begin VB.Form Sale 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox Party 
         ...
         Width           =   3495
      End
      Begin MSComCtl2.DTPicker SaleDate 
         Height          =   300
         Left            =   480
         TabIndex        =   1
         Top             =   600
   ...
   Begin MSComctlLib.ImageList ImageList1 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "Sale.frx":001C
            Key             =   ""
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "Sale.frx":0284
            Key             =   ""
         EndProperty
         BeginProperty ListImage3 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "Sale.frx":06DC
            Key             =   ""
         EndProperty
         BeginProperty ListImage4 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "Sale.frx":0AF8
            Key             =   ""
         EndProperty
         BeginProperty ListImage5 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "Sale.frx":0F2C
            Key             =   ""
         EndProperty
         BeginProperty ListImage6 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "Sale.frx":134C
            Key             =   ""
         EndProperty
         BeginProperty ListImage7 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "Sale.frx":1788
            Key             =   ""
         EndProperty
      EndProperty
   End
...
Private Sub CmdDel_Click()
    If (Len(Trim(m_ListID)) > 0 And Len(Trim(d_ListID)) > 0) Then
        ...
        If strAns = vbYes Then
            cnDatabase.Execute "update SaleDetail set IsActive = 0 where SaleDetailCode =" & d_ListID
            Call fillList
...
Private Sub Form_Load()
    ...
    SrDate = Date
    SrDate2 = Date
    FillCombo "Select Distinct t.ItemTypeCode, ItemTypeName from ItemType t inner join vwAvailableQty v on v.ItemTypeCode = t.ItemTypeCode where IsActive = 1 order by 2", Item_Type, "ItemTypeName", "ItemTypeCode"
...
Private Sub Item_Type_Click()
    If (Me.Item_Type.ListIndex > -1) Then
        ...
        FillCombo "Select Distinct i.ItemCode, i.ItemName from Item i inner join vwAvailableQty v on i.ItemCode = v.ItemCode where IsActive = 1 and i.ItemTypeCode = " & i, Item, "ItemName", "ItemCode"
    Else
        Me.Item.Clear
    End If
End Sub
...
Private Sub getValMaster()
    ...
    Set rstGetVal = FillRecordSet("Select SaleMasterCode, SaleMasterDate, PartyCode, ChallanCode From SaleMaster Where SaleMasterCode = " & m_ListID)
    If Not (rstGetVal.EOF) Then
        Me.SaleDate.value = IIf(IsNull(rstGetVal("SaleMasterDate")), Now, rstGetVal("SaleMasterDate"))
...
Private Sub getValDetail()
    Dim rstGetVal As New ADODB.Recordset
    Set rstGetVal = FillRecordSet("Select ItemTypeCode, ItemCode, Quantity, Rate From SaleDetail Where SaleDetailCode = " & d_ListID)
    If Not (rstGetVal.EOF) Then
...
Public Sub setValMaster()
    ...
    If (Len(Trim(m_ListID)) = 0) Then
        Set rstSave = FillRecordSet("select * from SaleMaster Where 1 = 2")
        rstSave.AddNew
        m_ListID = ValAutoNumber("SaleMaster", "SaleMasterCode")
        rstSave("SaleMasterCode") = m_ListID
    Else
       Set rstSave = FillRecordSet("select * from SaleMaster where SaleMasterCode =" & m_ListID)
    End If
    
    rstSave("SaleMasterDate") = Me.SaleDate.value
...
Public Sub setValDetail()
    ...
    If (Len(Trim(d_ListID)) = 0) Then
        Set rstSave = FillRecordSet("select * from SaleDetail Where 1 = 2")
        rstSave.AddNew
        d_ListID = ValAutoNumber("SaleDetail", "SaleDetailCode")
        rstSave("SaleDetailCode") = d_ListID
    Else
       Set rstSave = FillRecordSet("select * from SaleDetail where SaleDetailCode =" & d_ListID)
    End If
    ...
    rstSave("IsActive") = "1"
    rstSave("Cost") = getAvgCost(Me.Item.ItemData(Item.ListIndex))
...
Private Sub fillList()
    ...
    Set rstList = FillRecordSet("SELECT top 60 SaleDetailCode, SaleMaster.SaleMasterCode, PartyName, SaleMaster.PartyCode, SaleDetail.ItemTypeCode, ItemTypeName, SaleDetail.ItemCode, ItemName, Quantity, Rate " & _
                                "FROM (Party INNER JOIN (SaleMaster INNER JOIN SaleDetail ON SaleMaster.SaleMasterCode = SaleDetail.SaleMasterCode) ON Party.PartyCode = SaleMaster.PartyCode) INNER JOIN (ItemType INNER JOIN Item ON ItemType.ItemTypeCode = Item.ItemTypeCode) ON (SaleDetail.ItemCode = Item.ItemCode) AND (SaleDetail.ItemTypeCode = ItemType.ItemTypeCode) " & _
                                "Where SaleDetail.IsActive = 1 order by SaleMaster.SaleMasterCode desc, SaleDetailCode desc")
    lvwphase.ListItems.Clear
    If Not rstList.EOF Then
      Do While Not rstList.EOF
            Set lstItem = lvwphase.ListItems.Add( _
                   Text:=rstList!SaleDetailCode, _
                   Key:=CStr("Id=" & rstList!SaleDetailCode))
            With lstItem.ListSubItems
                 .Add Text:=rstList!SaleMasterCode
...
Private Sub SrItemType_Click()
    If Me.SrItemType.ListIndex > -1 Then
        ...
        FillCombo "Select Distinct i.ItemCode, ItemName from Item i inner join vwAvailableQty v on i.ItemCode = v.ItemCode where IsActive = 1 and i.ItemTypeCode = " & i, SrItem, "ItemName", "ItemCode"
    Else
        Me.SrItem.Clear
    End If
    Call SrfillList
...
Private Sub SrfillList()
    ...
    If dtChk.value = Checked Then
        'srdt = " And (SaleMasterDate between #" & Me.SrDate.value & " #  and #" & Me.SrDate2.value + 1 & " #)"
        srdt = " And (SaleMasterDate between Convert(datetime, '" & Me.SrDate.value - 1 & "')  and Convert(datetime, '" & Me.SrDate2.value + 1 & "'))"
    Else
        srdt = ""
    End If
    
    If PtChk.value = Checked And Me.srParty.ListIndex > -1 Then
        cbo1 = " And SaleMaster.partycode = " & Me.srParty.ItemData(Me.srParty.ListIndex)
    Else
        cbo1 = ""
    End If
    
    If ImTChk.value = Checked And Me.SrItemType.ListIndex > -1 Then
        cbo2 = " And SaleDetail.ItemTypeCode = " & Me.SrItemType.ItemData(Me.SrItemType.ListIndex)
    Else
        cbo2 = ""
    End If
    
    If ImChk.value = Checked And Me.SrItem.ListIndex > -1 Then
        cbo3 = " And SaleDetail.ItemCode = " & Me.SrItem.ItemData(Me.SrItem.ListIndex)
    Else
        cbo3 = ""
    End If
    
    sql = " SELECT top 60 SaleDetailCode, SaleMaster.SaleMasterCode, PartyName, SaleMaster.PartyCode, SaleDetail.ItemTypeCode, ItemTypeName, SaleDetail.ItemCode, ItemName, Quantity, Rate " & _
          " FROM (Party INNER JOIN (SaleMaster INNER JOIN SaleDetail ON SaleMaster.SaleMasterCode = SaleDetail.SaleMasterCode) ON Party.PartyCode = SaleMaster.PartyCode) INNER JOIN (ItemType INNER JOIN Item ON ItemType.ItemTypeCode = Item.ItemTypeCode) ON (SaleDetail.ItemCode = Item.ItemCode) AND (SaleDetail.ItemTypeCode = ItemType.ItemTypeCode) " & _
          " Where SaleDetail.IsActive = 1" & _
          srdt & _
          cbo1 & _
          cbo2 & _
          cbo3 & _
          " Order by SaleMaster.SaleMasterCode desc, SaleDetailCode desc"
